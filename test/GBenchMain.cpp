...
void BM_AyonLoggerConstruction(benchmark::State &state) {
  for (auto _ : state) {
...
void BM_AyonLoggerLogInfoNoKey(benchmark::State &state) {
  for (auto _ : state) {
...
void BM_AyonLoggerRegKey(benchmark::State &state) {
  for (auto _ : state) {
...
void BM_AyonLoggerUnRegKey(benchmark::State &state) {
  for (auto _ : state) {
...
void BM_AyonLoggerRegUnRegKey(benchmark::State &state) {
  for (auto _ : state) {
...
void BM_AyonLoggerLogInfoWrongKey(benchmark::State &state) {
  for (auto _ : state) {
...
void BM_AyonLoggerLogInfoRightKey(benchmark::State &state) {
  for (auto _ : state) {
...
void BM_AyonLoggerIsKeyActiveTrue(benchmark::State &state) {
  for (auto _ : state) {
...
void BM_AyonLoggerIsKeyActiveFalse(benchmark::State &state) {
  for (auto _ : state) {
...
static void BM_CompareNumbersRefBench(benchmark::State &state) {
  int a = state.range(0);
  int b = state.range(1);
  bool result;
...
int main(int argc, char **argv) {
  ...
  TestLogger = std::make_unique<AyonLogger>(
      AyonLogger::getInstance("/home/lyonh/ynput/DevEnv/projects/AyonCppLogger/"
