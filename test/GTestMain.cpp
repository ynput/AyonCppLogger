...
TEST(AyonLogger, AyonLoggerLogAllNoKey) {
  ...
  TestLogger = std::make_unique<AyonLogger>(
      AyonLogger::getInstance("test/LogFile.json"));
...
int main(int argc, char **argv) {
  ...
  TestLoggerGlob = std::make_unique<AyonLogger>(AyonLogger::getInstance(
      std::filesystem::current_path().string() + "/test/out/LogBench.json"));
